...
**Current version:** `3.9.0.2026.07.30` (proc) &nbsp;·&nbsp; `2026.07.30.10` (diag)
...
- **Wrapper 2026.07.30** - o2md.51 + o2md.40: report styling and provider modernization. Markdown reports gain severity emoji, a GitHub-alert top-issues callout, grade badges, and collapsible per-server sections (theme-agnostic -- the viewer controls .md theming); a self-contained dark-themed HTML twin now always ships alongside the .md (embedded dark CSS, severity-colored callouts, zero external assets -- nothing leaks from SAFE_TO_SHARE files). SQL client: prefers `Microsoft.Data.SqlClient` when resolvable, falls back to the runtime-bundled `System.Data.SqlClient` otherwise (install the modern package to opt in; no breakage without it).
- **Diag 2026.07.30.9** - o2md.24: I10 `RECOMMENDED_CONFIG` severity-precedence rewrite. The "just run this" output previously consumed only 4 finding categories and contradicted higher-severity findings on the same server. Now: CRITICAL > WARNING > baseline echo, with disagreements rendered as explicit `/* CONFLICT */` / `/* CAVEAT */` comment lines in the generated call; `SILENT_NOOP_RUN` blocks newly-enabling parallel and adds a version-gated upgrade caveat; `PRIORITY_PASS_EMPTY` halves `@ModificationThreshold` on 3.7.0+ (upgrade-first caveat below); I8 trend data can enable `@QueryStore=CPU` unless `QS_NOT_EFFECTIVE` contradicts; and the stale v2-era gate that suppressed `@MopUpPass` on parallel configurations is gone (why 31/40 parallel fleet servers never saw it).
- **Diag 2026.07.30.8** - Maintainer-decision batch 1 (o2md.19, o2md.14, o2md.33, o2md.35): W3 `STALE_BACKLOG` defers to C6 when every non-killed run confirmed zero processed (throughput advice is nonsense for pure no-op windows; mixed windows keep the kzx0 behavior); C4 caps at WARNING when either comparison window has under 5 runs, with thin-baseline + survivorship caveats in the evidence; AG-secondary by-design stops (`AG_SECONDARY`, StatsFound=0) are excluded from grading and surfaced as new INFO check I19 `AG_SECONDARY_BY_DESIGN` -- while the genuine secondary-host defect signature (found>0, processed=0) still fires everything; RS5/RS7 gain additive reason-token columns (`ImpactDataStatus`, `SampleDataStatus`) explaining legitimately-blank QS/sample numerics.
- **Diag 2026.07.30.7** - o2md.50: consume the proc v3.8.5 observability fields. W6 `EXCESSIVE_OVERHEAD` now subtracts the four measured self-idle-time buckets (`DelayWaitMs`/`AgRedoWaitMs`/`RetryBackoffMs`/`QueueClaimMs` from the END Summary XML) from the overhead percentage, so the figure is genuinely-unattributed time -- and the evidence states whether the worst run's figure had measured idle excluded or predates measurement (pre-3.8.5). W5 `QS_NOT_EFFECTIVE` family prefers the reliable per-row `FromMopUp` flag over the `QualifyReason=MOP_UP` inference on 3.8.5+ runs, so priority-pass leftovers mislabeled by the session-scoped mop-up flag now correctly count as priority-pass evidence; older runs keep the inference. Cache table gains `FromMopUp` via the existing auto-upgrade path.
- **3.8.5** - Observability: self-idle-time buckets + mop-up sort-order/tagging (o2md.46, o2md.49). The proc's own wait time (inter-stat delay, AG-redo recheck loop, retry backoff, parallel claim-cycle overhead) was invisible to Diag's overhead attribution -- four accumulators now emit in the END Summary XML as `DelayWaitMs`/`AgRedoWaitMs`/`RetryBackoffMs`/`QueueClaimMs` (omitted when zero). Mop-up discovery (all three sites) now honors `@SortOrder` for ROWS/PAGE_COUNT/DAYS_STALE/AUTO_CREATED/RANDOM instead of silently falling back to modification-counter order (QUERY_STORE ordering in mop-up remains a tracked design decision), and mop-up-discovered rows carry a per-row `FromMopUp` flag in per-stat ExtendedInfo -- unlike the session-scoped `QualifyReason=MOP_UP`, it cannot mislabel priority-pass leftovers, giving Diag a reliable way to tell "no QS data" from "QS not applicable to this pass".
- **Diag 2026.07.30.6** - Stale v3 parameter references in user-facing recommendation/@Help text, prompted by a live user report that `@CleanupOrphanedRuns` was appearing in Diag alert text even though v3 runs orphan cleanup automatically and removed the parameter. Auditing the same pattern found five more instances of the same defect class (v2 params absorbed into internal variables or removed outright, still referenced as if live): `@Help` Killed Run Detection topic, W15's `PARALLEL_ORPHAN_BACKLOG`/`STALE_QUEUE_SWEEP`/`AZURE_SQL_MI` recurring-code recommendations, W6 `EXCESSIVE_OVERHEAD`'s recommendation and code comment (`@DelayBetweenStats`, `@CommandLogRetentionDays`), and an o2md.18(c) code comment (`@LogSkippedToCommandLog`). Two of the six were self-inflicted -- introduced by the immediately-prior 2026.07.30.4 batch (o2md.27) while fixing W6 -- caught only because the live report prompted auditing for the defect class rather than trusting the single reported instance.
- **Diag 2026.07.30.5** - Redundant/tautological findings + obfuscation hardening (o2md.43, o2md.44, o2md.28, o2md.36): W7 `HIGH_IMPACT_STATS_DEPRIORITIZED` now only fires when QUERY_STORE sort is actually active (was firing on ~half of any top-10 list by construction, unconditionally recommending a sort order that might already be on); I9 `WORKLOAD_CONCENTRATION` reframed as neutral descriptive text instead of an action item (Pareto principle, not a defect); W5/I6/I8's redundant "no QS data" (three findings, one fact, ~45 redundant/446 in the fleet sweep) collapsed to a single finding; obfuscation tokens widened from 6-hex (Schema was actually 4-hex, the confirmed cause of a fleet-wide collision) to 8-hex uniformly, and an unseeded run now auto-generates a random salt instead of hashing unsalted.
...
- **3.8.3.2026.07.30** - Query Store diagnostics + priority-pass/mop-up inversion warning (o2md.6, QuickieStore comparison): `wait_stats_capture_mode` is now checked before WAITS/WAIT_CPU enrichment (capture-mode OFF previously produced silent zero enrichment indistinguishable from "no matching workload"); READ_ONLY Query Store state now decodes `readonly_reason` (storage limit with current/max MB, AG secondary, single-user/emergency mode) instead of one generic message; `query_capture_mode` is surfaced when NONE/CUSTOM may be silently biasing which queries QS-based prioritization even sees; the forced-plan-failure-delta warning now samples `last_force_failure_reason_desc` so it's self-contained instead of "go investigate". All four are additive diagnostics -- no change to qualification, ordering, or enrichment logic. Also: the priority pass qualifying 0 stats while mop-up (raw `modification_counter > 0`, no threshold filtering) finds N > 0 is never legitimate -- it means priority-pass qualification is broken, not that the fleet has nothing to do. Now detected and surfaced via `@WarningsOut`/`@WarningsCodesOut` (`PRIORITY_PASS_ZERO_MOPUP_FOUND`) instead of mop-up silently absorbing the gap.
- **Diag 2026.07.30.1** - o2md.42: named `PRIMARY KEY` constraints on 5 temp tables (`#stat_updates`, `#recommendations`, `#run_version_keys`, `#forced_plans`, `#qs_efficacy`) unnamed. Temp table *names* get a per-session uniquifier; named *constraints* on those tables don't, so two concurrent Diag sessions on the same instance collided on the constraint name and the second session's `CREATE TABLE` failed outright.
- **3.8.2.2026.07.28** - **SILENT NO-OP: fleet analysis batch** (o2md.1/.2/.3/.9): found by a 40-server production diagnostic sweep where ~50 nightly maintenance windows reported SUCCESS while processing zero statistics. Root cause: lazy mop-up branches that discover 0 stats for a claimed table fell through without a `CONTINUE`, abandoning every remaining queued table with `@stop_reason = COMPLETED`. Also fixed: `@remaining_stats` reporting 0 when a worker joined an existing parallel queue, and `StatsFound > 0 / StatsProcessed = 0` now grades `Status = ERROR` and returns 2 so SQL Agent fails the job (previously only caught by the separate diagnostic tool, so no-ops went unnoticed for weeks).
- **Diag 2026.07.28.1** - Fleet analysis batch (o2md.10/11/13/16/17/18/22): checks that asserted more than their evidence supported. Includes a 100% false-positive fix (`CRITICAL_TABLES_NO_MATCH` misreading FOR XML `XSINIL`-serialized empty strings as "set"), a root-cause attribution fix for `SILENT_NOOP_RUN`, and a fix for `QS_PERFORMANCE_TREND` reporting impossible percentages from unbounded ratio averaging.
- **Diag 2026.07.25.1** - `@Ansi` opt-in extended: also renders the Executive Dashboard and Recommendations as colorized text on the Messages stream, in addition to the normal result sets.
- **3.8.1.2026.07.25** - MONITORING: per-stat CONTEXT_INFO refresh (sp_StatUpdate-03i3 / gh-538). The static CONTEXT_INFO set at proc entry (DB/Job/Preset only) is now refreshed every loop iteration with a compact `SU|<n>/<total>|<schema.table.stat>|Run:<label>` payload, so `sp_WhoIsActive` mid-run shows which stat is currently being processed instead of just the entry-time snapshot.
